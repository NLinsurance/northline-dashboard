...
app.get('/api/pacing', (req, res) => {
  ...
  const byAgent = agents.map(agent => {
    ...
    return { agent, ...computePacing(agentSales, today, holidaySet) };
...
function buildReport(today, mode) {
  ...
  byAgent.forEach(a => {
    lines.push(`  ${a.agent} — Today: ${money(a.today[mode])} (${a.today.policies} pol) | MTD: ${money(a.mtd[mode])} | Run Rate: ${money(a.runRate[mode])}`);
  });
